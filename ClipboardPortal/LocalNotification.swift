...
func showClipboardContentNotification(_ content: ClipboardContent) async {
    guard await SettingsStore.shared.settingsData.notificationsEnabled else { return } // Only send notifications if enabled
    ...
    notificationContent.sound = await !SettingsStore.shared.settingsData.receiveSoundEnabled ? UNNotificationSound.default : nil // No sounds if app plays own sounds
